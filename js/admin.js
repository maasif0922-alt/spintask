...
const Admin = {
    ...
    requestTransaction(userId, type, amount, address, proof = null) {
        ...
        if (typeof db !== 'undefined' && db !== null) {
            db.ref('transactions/' + tx.id).set(tx);
        }
        ...
        if (index !== -1 && txs[index].status === 'pending') {
            ...
            if (newStatus === 'approved') {
                ...
                if (uIndex !== -1) {
                    ...
                    // Sync to Realtime Database
                    if (typeof db !== 'undefined' && db !== null) {
                        db.ref('users/' + txs[index].userId).update({ balance: users[uIndex].balance });
                    }
                }
            }
            ...
            // Sync to Realtime Database
            if (typeof db !== 'undefined' && db !== null) {
                db.ref('transactions/' + txId).update({ status: newStatus });
            }
